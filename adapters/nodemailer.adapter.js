...
class NodeMailerAdapter {
    ...
    send = async (name, email, role, token_email) => {
        let info = await this.#transporter.sendMail({ ...this.#mailpayload, to: email, html: templateEmail(name, email, role, `http://${config.app.domain}:${config.app.port}/account/email/confirm/recive/success?token_email=${token_email}`) })
        // let info = await this.#transporter.sendMail({ ...this.#mailpayload, to: email, html: templateEmail(name, email, role, `http://${config.frontend.domain}:${config.frontend.port}/alert/success?token_email=${token_email}`) })
